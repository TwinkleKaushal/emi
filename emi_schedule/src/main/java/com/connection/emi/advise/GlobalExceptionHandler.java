...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ValidationError> inavlidArgument(ConstraintViolationException ex, WebRequest req) {
		...
		ve = ValidationError.builder().status(HttpStatus.UNPROCESSABLE_ENTITY.value()).errorMessage(msg)
				.path(req.getDescription(false)).build();

		log.error(gson.toJson(
				LoggingResponseModel.builder().message(msg).messageTypeId(MessageTypeConstant.ERROR.getMessage())
	...
	public ResponseEntity<ValidationError> inavlidArgumentexception(NumberFormatException ex, WebRequest req) {
		...
		ValidationError ve = ValidationError.builder().status(HttpStatus.UNPROCESSABLE_ENTITY.value()).errorMessage(msg)
				.path(req.getDescription(false)).build();
